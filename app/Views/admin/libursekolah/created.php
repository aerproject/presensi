...
<div class="container-fluid mt-4">
  <h3 class="mb-3">Tambah Libur Sekolah</h3>

  <?php if (session()->getFlashdata('error')): ?>
    <div class="alert alert-danger"><?= esc(session()->getFlashdata('error')) ?></div>
  <?php endif; ?>

  <form action="<?= base_url('/admin/libursekolah/store') ?>" method="post">
    <?= csrf_field() ?>
    ...
    <div class="mb-3">
      ...
      <input type="text" class="form-control" id="tahun_pelajaran" name="tahun_pelajaran" 
             value="<?= esc($tahun_pelajaran) ?>" readonly>
    </div>

    <div class="mb-3">
      <label for="semester" class="form-label">Semester</label>
      <input type="text" class="form-control" id="semester" name="semester" 
             value="<?= esc($semester) ?>" readonly>
    </div>
