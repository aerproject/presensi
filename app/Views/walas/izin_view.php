...
<h4 class="mb-4">Data Izin / Sakit Siswa</h4>
...
<div class="card">
  <div class="card-body">
    <div class="d-flex justify-content-between align-items-center mb-3">
      <h5 class="card-title mb-0">Daftar Surat Izin/Sakit</h5>
      <!-- Dropdown perPage -->
      ...
      <tbody>
        <?php if (!empty($izin)): ?>
          ...
          <tr>
            ...
            <td><?= ucfirst(esc($row['jenis'])) ?></td>
            <td><?= ucfirst(esc($row['status'])) ?></td>
            <td><?= esc($row['isi_pesan']) ?></td>
            <td>
              <?php if (!empty($row['upload_surat'])): ?>
                <a href="<?= base_url('uploads/surat/'.$row['upload_surat']) ?>" target="_blank">Lihat Surat</a>
              <?php else: ?>
                -
              <?php endif; ?>
            </td>
        ...
        <?php else: ?>
          <tr>
            <td colspan="7" class="text-center">Belum ada surat izin/sakit</td>
          </tr>
        <?php endif; ?>
      </tbody>
    </table>
